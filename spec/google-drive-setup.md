...
remote (gon9a-book)
  └─ video-digest task
        ├─ Obsidian Vault の同期先へコピー
        │     └─ docs_obsidian/20_news/owlclaw/video/digest_YYYYMMDD.mp4
...
1. mp4 を生成
2. Obsidian Vault の `docs_obsidian/20_news/owlclaw/video/` に同期用コピーを保存
3. Drive にアップロード（同名ファイルがあれば上書き）
4. 「リンクを知っている人は閲覧可」を付与
5. Slack 通知に webViewLink を埋め込む

Drive API の認証やアップロードに失敗しても、同期用コピーは残る。Slack 通知には
リモートマシン固有の絶対パスではなく `Obsidian: owlclaw/video/...` を表示する。
...
mp4 は 1 本 ~2MB 程度。`retention_days: 7` でローカルと Obsidian Vault の同期用コピーは
自動削除されるが、**Drive API で別途アップロードしたファイルは自動削除されない**。
必要に応じて Drive 上の `owlclaw/video-digest/` を定期掃除する。
