...
def _publish_video_to_obsidian(
    local_mp4: Path,
    obsidian_subdir: str,
    retention_days: int,
) -> tuple[Path, str]:
    """動画をGoogle Drive同期対象のObsidian Vault配下へコピーする。"""
    vault = os.environ.get("OBSIDIAN_VAULT")
    if not vault:
        raise RuntimeError("OBSIDIAN_VAULT が未設定です")

    relative_dir = Path(obsidian_subdir)
    if relative_dir.is_absolute() or ".." in relative_dir.parts:
        raise ValueError(
            f"obsidian_subdir はVault内の相対パスで指定してください: {obsidian_subdir}"
        )

    dest_dir = Path(vault) / "docs_obsidian" / "20_news" / relative_dir
    dest_dir.mkdir(parents=True, exist_ok=True)
    _purge_old_videos(dest_dir, retention_days)

    dest_mp4 = dest_dir / local_mp4.name
    shutil.copy2(local_mp4, dest_mp4)
    relative_path = str(relative_dir / local_mp4.name)
    print(f"  Obsidian video written: {dest_mp4}", file=sys.stderr)
    return dest_mp4, relative_path
...
def _dispatch_video_output(output: dict, task_dir: Path, date: str) -> None:
    ...
    # Obsidian Vault は Google Drive 同期対象。remote 固有パスだけに動画を残さない。
    obsidian_relative_path: str | None = None
    obsidian_subdir = output.get("obsidian_subdir")
    if obsidian_subdir:
        _, obsidian_relative_path = _publish_video_to_obsidian(
            out_mp4,
            str(obsidian_subdir),
            retention_days,
        )
    ...
    if output.get("slack_notify"):
        if obsidian_relative_path:
            slack_msg = (
                ...
                ":memo: Google Drive 同期後に Obsidian Vault から再生できます"
            )
        else:
            slack_msg = (
                f":movie_camera: *動画ダイジェスト生成完了* `{date}`\n"
