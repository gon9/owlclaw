...
def _resolve_ai_config(task: dict) -> dict[str, str | None]:
    """タスク定義から AI provider/model 設定を解決する。"""
    cfg = task.get("ai") or {}
...
def _resolve_visual_mode(task: dict) -> str:
    """video.visual_mode を解決する。未指定なら AI 設定から既定値を選ぶ。"""
    video_cfg: dict = task.get("video") or {}
    if "visual_mode" in video_cfg:
        return str(video_cfg["visual_mode"]).lower()
    return _default_visual_mode_for_ai(_resolve_ai_config(task))


def _relative_task_file(filename: object) -> Path:
    """task input で指定された相対ファイル名を安全に Path 化する。"""
    path = Path(str(filename))
    if path.is_absolute() or ".." in path.parts:
        raise ValueError(f"input file must be a relative path under tmp/<task>: {filename}")
    return path


def _input_extra_files(input_cfg: dict) -> list[str]:
    """input.extra_files を正規化する。"""
    extra_files = input_cfg.get("extra_files") or []
    if not isinstance(extra_files, list):
        raise ValueError("input.extra_files must be a list")
    return [str(item) for item in extra_files]


def _copy_task_artifact(src_task: str, src_file: str, task_dir: Path) -> str:
    """別タスクの成果物を task_dir に provenance 付きでコピーして内容を返す。"""
    src_rel = _relative_task_file(src_file)
    src_path = PROJ / "tmp" / src_task / src_rel
    if not src_path.exists():
        raise FileNotFoundError(
            f"input source {src_path} が見つかりません。先に {src_task} を実行してください。"
        )
...
def _copy_from_task_inputs(input_cfg: dict, task_dir: Path) -> str:
    """input.from_task の主入力と追加参照ファイルを task_dir に配置する。"""
    src_task = str(input_cfg["from_task"])
    src_file = str(input_cfg.get("file", "note_draft.md"))
    primary_content = _copy_task_artifact(src_task, src_file, task_dir)

    for extra_file in _input_extra_files(input_cfg):
        if _relative_task_file(extra_file) == _relative_task_file(src_file):
            continue
        _copy_task_artifact(src_task, extra_file, task_dir)

    return primary_content


def _input_files(task: dict) -> set[Path]:
    """タスク入力として配置されるファイル名を返す。"""
    input_cfg: dict = task.get("input") or {}
    input_file = input_cfg.get("file", "events.md")
    files = {_relative_task_file(input_file)}
    files.update(_relative_task_file(extra_file) for extra_file in _input_extra_files(input_cfg))
    return files


def _clear_stale_ai_outputs(task: dict, task_dir: Path) -> None:
    """AI が今回書かなかった場合に古い成果物を再利用しないよう消しておく。"""
    output_files: dict[str, Path] = {
        "obsidian": Path("note_draft.md"),
        "slack": Path("slack.txt"),
        "video": Path("slides.json"),
    }
    input_files = _input_files(task)
    for output in task.get("outputs", []):
        rel_path = output_files.get(str(output.get("type")))
        if rel_path is None or rel_path in input_files:
            continue
...
def _build_claude_prompt(task: dict, task_dir: Path) -> str:
    ...
    extra_files = _input_extra_files(input_cfg)
    if extra_files:
        lines.append("   ※ 追加の参照ファイルも Read すること")
        for extra_file in extra_files:
            extra_path = _relative_task_file(extra_file)
            lines.append(f"   - Read `{task_dir / extra_path}`")
    ...
    if "video" in outputs:
        visual_mode = _resolve_visual_mode(task)
        lines.append(
            f"   ※ visual_mode = `{visual_mode}`。"
            "ニュース本文スライドの表現方式はこの設定を必ず優先すること"
        )
    step = 5
...
def _invoke_claude(
    ...
    *,
    ...
    cmd = ["claude", "--print", "--allowedTools", allowed_tools]
    if model:
...
def _video_output_config(task: dict) -> dict:
    """task.outputs から video 出力設定を返す。無ければ空 dict。"""
    for output in task.get("outputs", []):
...
def _render_and_validate_slides(task_dir: Path, date: str) -> list[Path]:
    """slides.json から PNG を生成し、スライド成果物だけ検証する。"""
...
def _dispatch_debug_slides(task: dict, task_dir: Path, date: str) -> Path:
    """スライドだけ生成して Google Drive に upload するデバッグ出力。"""
    slides_json = task_dir / "slides.json"
    if not slides_json.exists() or slides_json.stat().st_size == 0:
        raise RuntimeError(f"slides.json がありません: {slides_json}")

    pngs = _render_and_validate_slides(task_dir, date)
    if not pngs:
        raise RuntimeError(f"スライド PNG が生成されませんでした: {task_dir / 'slides'}")

    output = _video_output_config(task)
    base_folder = output.get("drive_folder", "owlclaw/video-digest")
    run_stamp = datetime.now(JST).strftime("%Y%m%d-%H%M%S")
    folder = f"{base_folder}/debug-slides/{date}-{run_stamp}"

    from tools.upload_drive import upload_to_drive  # noqa: PLC0415

    links: list[tuple[str, str]] = []
    print(f"  Debug slides Drive upload: {folder}/", file=sys.stderr)
    for path in [slides_json, *pngs]:
        result = upload_to_drive(path, folder_path=folder)
        link = result.get("webViewLink") or ""
        links.append((path.name, link))
        print(f"    {path.name}: {link}", file=sys.stderr)

    out = task_dir / "debug_slides_drive.txt"
    lines = [f"# Debug slides {date}", "", f"Drive folder: {folder}", ""]
    lines.extend(f"- {name}: {link}" for name, link in links)
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return out


def _purge_old_video_artifacts(task_dir: Path, retention_days: int) -> None:
    """動画生成の成果物・中間ファイルを retention_days に従って削除する。"""
    ...
    seg_dir = task_dir / "_segments"
    if seg_dir.exists():
        mtime = datetime.fromtimestamp(seg_dir.stat().st_mtime, tz=UTC)
        if mtime < cutoff:
            for path in seg_dir.glob("*"):
                if path.is_file():
                    path.unlink(missing_ok=True)
            try:
                seg_dir.rmdir()
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            print(
                (
                    f"  retry: {label} failed (attempt={attempt}/{max_attempts}): {exc}"
                    f" → sleep {delay:.1f}s"
                ),
                file=sys.stderr,
            )
...
def _dispatch_video_output(output: dict, task_dir: Path, date: str) -> None:
    ...
    # 古い成果物を先に削除（ストレージ圧迫対策、既定: 7日）
    retention_days = int(output.get("retention_days", 7))
    _purge_old_video_artifacts(task_dir, retention_days)

    slides_dir = task_dir / "slides"
    audio_dir = task_dir / "audio"
    out_mp4 = task_dir / f"digest_{date.replace('-', '')}.mp4"

    retry_attempts = int(os.environ.get("OWLCLAW_VIDEO_RETRY_MAX_ATTEMPTS", "3"))
    base_delay = float(os.environ.get("OWLCLAW_VIDEO_RETRY_BASE_DELAY_SECONDS", "5"))
    ...
    _retry(
        lambda: _render_and_validate_slides(task_dir, date),
        label="render_slides",
    ...
    _retry(
        lambda: subprocess.run(
            [
                py,
                str(scripts_dir / "render_audio.py"),
                str(slides_json),
                str(audio_dir),
            ],
            check=True,
    ...
    try:
        _retry(
            lambda: subprocess.run(
                [
                    py,
                    str(scripts_dir / "compose_video.py"),
                    str(slides_json),
                    str(slides_dir),
                    str(audio_dir),
                    str(out_mp4),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "video タスク用: slides.json と PNG だけ生成して Drive に upload し、"
            "音声/動画/Slack はスキップ"
        ),
    )
    parser.add_argument(
        "--visual-mode",
        choices=["imagegen", "ppt", "html"],
        default=None,
        help="video タスク用: visual_mode を一時的に上書き",
    )
    ...
    if args.visual_mode:
        task.setdefault("video", {})["visual_mode"] = args.visual_mode
    ...
    if src_list:
        ...
        try:
            events_md = _copy_from_task_inputs(input_cfg, task_dir)
    ...
    if args.debug_slides:
        ...
        out = _dispatch_debug_slides(task, task_dir, date)
        print(f"=== [{task_id}] debug slides 完了: {out} ===", file=sys.stderr)
        return
